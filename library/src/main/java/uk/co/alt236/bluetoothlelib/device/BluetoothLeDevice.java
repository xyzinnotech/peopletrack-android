    ...
    public double getRunningAverageRssi() {
        ...
        //final Long aLong;
            for (final Long aLong : mRssiLog.keySet()) {
                ...
                s= s+ "  ";
                s= s+ mRssiLog.get(aLong);
            }
        }

        //Toast.makeText(BluetoothLeDevice.this,"Your Message", Toast.LENGTH_LONG).show();

        a= java.lang.Math.pow(10,(-1*sum/count - 58)/25);
        distance = (double) Math.round(a * 100) / 100;
        if (count > 0) {
            return distance;
        } else {
            return 0;
